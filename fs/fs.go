...
// cleanup the store, removes expired items
func (m *File) cleanup() {
	m.mu.Lock()
	defer m.mu.Unlock()
	...
	if m.dirty {
		...
		if err == nil {
			...
			go func() {
				...
				if err != nil {
					m.log.Printf("error while writing to file %q: %v", m.cfg.Path, err)
				}
			}()
		}
	}
